    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group col-md-2"><input type="text" class="form-control" id="studentRollNum" readonly></div>
            <div class="form-group col-md-4"><input type="text" class="form-control" id="Student Name" placeholder="Student Name" readonly></div>
           </div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-success" onclick="generate($('#studentRollNum').val(),'generate')">Generate Report</button>
...
<script>
      ...
      "columns": [
        ...
        {data: "attendance_date"},
        {
          ...
          render: function (data){
            return `<button type="button" onclick="generate(${data},'fetch')" class="btn btn-outline-info btn-sm">Generate</button>`
          }
        }
      ],
      "columnDefs": [
  ...
  function generate(id,key){
    ...
    switch (key){
      case "fetch":
        $("#reportModal").modal()
        $("#studentRollNum").val(student_id)
        break;
      case "generate":
        if(from_date === '' && to_date === ''){
          alert("Fields are Missing")
        }else{
          window.open("generate_attendance.php?key=generate&student_id="+id+"&from_date="+from_date+"&to_date="+to_date)
        }
        break;
    }
  }
